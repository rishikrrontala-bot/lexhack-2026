# CLAUDE.md: Rishik Rontala's entry for LexHack 2026
...
**Deadline: Sun Sep 27, 2026 · 5:00 PM EDT.** Event page: https://lexhack-2026.devpost.com/
...
- **GitHub Pages deploys automatically from whatever branch you push** (`.github/workflows/pages.yml` builds `dist/`, `build/`, `out/` or `site/`, or a root `index.html`). Live URL: **https://rishikrrontala-bot.github.io/lexhack-2026/**. After each deploy, check the Actions run succeeded and load the URL headlessly.
- Finish on `main` (judges read the default branch). Try `git push origin HEAD:main`. If that's refused, open a PR and merge it with the REST API (`gh api -X POST repos/rishikrrontala-bot/lexhack-2026/pulls -f title=… -f head=<branch> -f base=main`, then `gh api -X PUT repos/rishikrrontala-bot/lexhack-2026/pulls/<n>/merge -f merge_method=squash`). If that's refused too, make "merge the PR" step 1 of HANDOFF.md.
